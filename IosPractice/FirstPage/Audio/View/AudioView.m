...
- (instancetype)initWithFrame:(CGRect)frame{
    ...
    if(self){
        ...
        [songListTable registerClass:[UITableViewCell class] forCellReuseIdentifier:@"songListCell"];
...
- (void)layoutSubviews{
    ...
    self.songListTable.frame = CGRectMake(0, offset, self.frame.size.width, self.frame.size.height - offset);
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    if(tableView.tag == 1001){
        ...
        cell.textLabel.text = [self.functionArray objectAtIndex:indexPath.row];
